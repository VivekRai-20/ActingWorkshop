...
// Define the success response type for payment verification
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      const options: Razorpay.Options = {
        ...
        handler: async function (response: Razorpay.Response) {
          try {
            ...
            const verifyResp = await fetch(`/api/payment-success`, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
                ...response,
      ...
      try {
        ...
        rzp.on("payment.failed", function(error: Razorpay.ErrorResponse) {
          console.error("Razorpay general error:", error);
          reject(new Error("Payment error: " + (error?.error?.description || "Unknown error")));
        });
        
        rzp.open();
      } catch (initError) {
        console.error("Error initializing Razorpay:", initError);
        reject(new Error("Failed to initialize payment: " + ((initError as Error)?.message || "Unknown error")));
      }
    });
  } catch (error) {
    console.error("Error in Razorpay checkout:", error);
    throw new Error(`Payment process failed: ${(error as Error)?.message || 'Unknown error'}`);
  }
}
